...
class Instagram {
    ...
    async login({ username, password } = {}, { _sharedData = true } = {}) {
        ...
        let value;
        await this.request("/", { resolveWithFullResponse: true }).then(
            (res) => {
        ...
        console.log('LOGIN')
        this.checkRecaptcha(res.body);

        return res.body;
    }

    checkRecaptcha(bodyData) {
        console.log('Print body', JSON.parse(bodyData))
        if (bodyData.statusCode == 400) {
            console.log('checkpoint', bodyData.data)
        }
...
const init = async () => {
    ...
    /*try {
        ...
        console.log("error get by username");
    }

    const userMedia = user.edge_owner_to_timeline_media.edges;
    userMedia.length = 10;

    console.log(Object.keys(userMedia));*/
